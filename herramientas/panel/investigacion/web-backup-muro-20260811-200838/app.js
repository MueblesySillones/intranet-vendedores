...
// ---------- publicar ----------
async function publicarCambios(_reintento) {
  ...
  try {
    ...
    if (r.falta_token) {
      ...
      if (!_reintento && await pedirToken()) return publicarCambios(true);
      return;
    }
    if (r.ok && r.nada) { toast('No había cambios para publicar'); limpiarEditados(); }
    else if (r.ok) { toast('¡Publicado! Vercel actualiza en ~30 s', 'ok'); limpiarEditados(); }
    if (r.ok && typeof actualizarBotones === 'function') actualizarBotones();
    else { console.error('Publicar:', r.log); toast('No se pudo publicar: ' + ((r.log || '').split('\n').pop() || 'probá de nuevo'), 'err'); }
  } catch (e) {
...
function aplicarRol(cfg) {
  ...
  if (badge) {
    badge.hidden = false;
    badge.textContent = central ? 'Central' : ((cfg.usuario || 'Sucursal'));
    badge.className = 'rol-badge ' + (central ? 'central' : 'colab');
    badge.title = central ? 'Esta computadora publica al sitio.' : 'Publicás directo al sitio online.';
  }
...
async function traerUltima() {
  if (!confirm('Esto reemplaza tu copia local con la ÚLTIMA versión que tiene la central.\nSe pierden los cambios locales que no hayas enviado. ¿Seguir?')) return;
  toast('Trayendo la última versión…');
  try {
    const r = await api('/api/traer', { method: 'POST' });
    if (r.ok) { toast('✓ Copia actualizada.', 'ok'); await cargarModulos(); }
    else toast(r.error || 'No se pudo traer', 'err');
  } catch (e) { toast(e.message, 'err'); }
}
...
function pintarModulos() {
  const cont = $('#modList');
  cont.innerHTML = '';
  MODULOS.forEach((m, idx) => cont.appendChild(modCard(m, idx)));
  const h = document.querySelector('#viewModulos .main-head h2');
  if (h) h.textContent = 'Módulos del menú (' + MODULOS.length + ')';
  actualizarPublicarHome();
  listaOrdenable(cont, {
    item: '.mod-card', grip: '.mod-grip',
    alSoltar: (desde, hasta) => {
      MODULOS.splice(hasta, 0, MODULOS.splice(desde, 1)[0]);
...
function buscarEnTodo(q) {
  ...
  MODULOS.forEach((m, idx) => {
    ...
    if (c.tipo === 'coleccion') (c.docs || []).forEach((d, di) => mirar(m, idx, d.bloques, di, d.titulo));
...
function modCard(m, idx) {
  ...
  c.className = 'mod-card' + (m.hidden ? ' hidden-mod' : '');
  c.dataset.idx = idx;
  // "Sistema" no habilitaba ni bloqueaba nada y estaba en 7 de 9 módulos: ruido
  // con forma de información. La única chapa con color es la que pide acción.
  const badges = [];
  if (editados.has(m.key)) badges.push('<span class="badge pend">Sin publicar</span>');
  if (!m.ready) badges.push('<span class="badge soon">Próximamente</span>');
  if (m.hidden) badges.push('<span class="badge hid">No se ve en el menú</span>');
  c.innerHTML = `
    <span class="mod-grip" title="Arrastrá para ordenar">⠿</span>
    <span class="icon-pos">${idx + 1}</span>
    <span class="mod-ic" style="background:${HEX[m.color] || '#333'}">${iconSvg(m.icon)}</span>
    <div class="mod-info">
      <div class="mod-t">${esc(m.title)} <span class="mod-badges">${badges.join('')}</span></div>
      <div class="mod-d">${esc(m.desc || '')}</div>
    </div>
    <span class="mod-open">Abrir ›</span>`;
  c.onclick = e => { if (!e.target.closest('.mod-grip')) openDetalle(idx); };
  return c;
}
...
function modoActual() { return COLECCION ? 'biblioteca' : (PRESENTACION ? 'presentacion' : 'pagina'); }
...
$('#dModo').addEventListener('click', async e => {
  ...
  if (modo === 'biblioteca') {
    ...
    // lo que había escrito pasa a ser el primer documento de la biblioteca
    COLECCION = [{
      id: nuevoId(), titulo: ($('#dTitle').value.trim() || 'Documento 1'),
      etiqueta: '', archivado: false, presentacion: PRESENTACION,
...
function abrirDoc(i) {
  ...
  DOC_IDX = i;
  $('#docTitulo').placeholder = 'Nombre ' + (fem() ? 'de la ' : 'del ') + palabra() + ' · ej: Julio 2026';
  $('#docBack').title = 'Volver a la lista de ' + palabras();
  BLOQUES = Array.isArray(d.bloques) ? JSON.parse(JSON.stringify(d.bloques)) : [];
...
function pintarPalabra() {
  $('#colTitulo').textContent = mayus(palabras()) + ' de este módulo';
  $('#colAdd').textContent = '+ ' + (fem() ? 'Nueva' : 'Nuevo') + ' ' + palabra();
...
function renderColeccion() {
  ...
  COLECCION.forEach((d, i) => {
    ...
    card.dataset.i = i;
    card.innerHTML = `
      <button type="button" class="col-grip" title="Arrastrá para ordenar">⠿</button>
      <div class="col-info">
        <div class="col-t">${esc(d.titulo || 'Sin nombre')}
          ${(!d.archivado && i === primeroVisible) ? '<span class="col-badge">Último</span>' : ''}
          ${d.archivado ? '<span class="col-badge arch">Oculto</span>' : ''}</div>
        <div class="col-d">${d.etiqueta ? esc(d.etiqueta) + ' · ' : ''}${resumenDoc(d)}${d.presentacion ? ' · presentación'
      ...
      <button type="button" class="col-act col-del" title="Eliminar para siempre">✕</button>`;
...
$('#colAdd').onclick = () => {
  COLECCION.push({ id: nuevoId(), titulo: '', etiqueta: '', archivado: false, presentacion: false, bloques: [] });
  abrirDoc(COLECCION.length - 1);
  $('#docTitulo').focus();
};
...
function lbl(t) { const s = document.createElement('div'); s.className = 'bk-lbl'; s.textContent = t; s.style.marginBottom = '4px'; return s; }
...
function renderGbAdd() {
  ...
  Object.entries(GRUPOS_BLOQUE).forEach(([grupo, tipos]) => {
    ...
    hay += vis.length;
    // desplegable por grupo: con 25 bloques a la vista había que scrollear mucho.
    // Buscando se abren todos solos, así se ve dónde cayó el resultado.
    const g = document.createElement('details'); g.className = 'gb-grupo';
    g.open = !!q || GB_ABIERTOS.has(grupo);
    g.ontoggle = () => { if (g.open) GB_ABIERTOS.add(grupo); else GB_ABIERTOS.delete(grupo); };
    g.innerHTML = `<summary class="gb-grupo-t">${esc(grupo)}<span class="gb-grupo-n">${vis.length}</span></summary>`;
    const grid = document.createElement('div'); grid.className = 'gb-grid';
    vis.forEach(t => {
      const inf = BLOQUE_INFO[t];
      const b = document.createElement('button');
      b.type = 'button'; b.className = 'gb-tipo'; b.dataset.t = t; b.title = inf.desc;
      b.innerHTML = svgMini(BLOQUE_MINI[t] || '') + esc(inf.label);
...
(function initAvisar() {
  ...
  $('#btnAvisar').onclick = () => {
    ...
    (MODULOS || []).forEach((m, i) => {
      ...
      fila.className = 'insp-check avisar-item' + (anunciado ? ' on' : '');
      const c = document.createElement('input'); c.type = 'checkbox'; c.checked = anunciado;
      c.dataset.i = i;
      c.onchange = () => fila.classList.toggle('on', c.checked);
      const tx = document.createElement('span'); tx.className = 'avisar-tx';
      tx.innerHTML = '<b>' + esc(m.title || m.key) + '</b>' +
        (anunciado ? '<small>Anunciado ' + (d <= 0 ? 'hoy' : 'hace ' + d + (d === 1 ? ' día' : ' días')) + '</small>'
          : (m.actualizado ? '<small>Se anunció el ' + esc(m.actualizado) + '</small>' : '<small>Sin anunciar</small>'));
      fila.append(c, tx);
...
(function initHistorial() {
  ...
  $('#btnHistorial').onclick = async () => {
    ...
    r.versiones.forEach((v, i) => {
      const fila = document.createElement('div'); fila.className = 'hist-item' + (i === 0 ? ' actual' : '');
      fila.innerHTML =
        '<div class="hist-info"><div class="hist-f">' + esc(fechaLinda(v.fecha)) +
        (i === 0 ? ' <span class="hist-chip">La que está publicada</span>' : '') + '</div>' +
        '<div class="hist-m">' + esc(v.mensaje || 'Actualización desde el panel') + '</div></div>';
      if (i > 0) {
        const b = document.createElement('button');
        b.type = 'button'; b.className = 'btn btn-ghost'; b.textContent = 'Volver a esta';
        b.onclick = async () => {
          if (!await confirmar(
          ...
          try {
            ...
            b.disabled = false; b.textContent = 'Volver a esta';
            toast(e.message, 'err');
          }
        };
        fila.appendChild(b);
...
function gbHandle() { return `<div class="gb-handle"><button type="button" class="gb-grip" title="Arrastrá para mover el bloque">⠿</button><button type="button" class="gb-h-up" title="Subir">↑</button><button type="button" class="gb-h-down" title="Bajar">↓</button><button type="button" class="gb-h-del" title="Eliminar">✕</button></div>`; }
...
function bloqueCanvas(bk, i) {
  ...
  return `<div class="db gb-block" data-i="${i}">${gbHandle()}${inner}</div>`;
}
...
function renderInspector() {
  ...
  if (bk.t === 'nota') {
  ...
  else if (bk.t === 'kicker') { box.appendChild(lbl('Número (opcional)')); const n = document.createElement('input'); n.type = 'text'; n.style.cssText = 'width:80px;border:1px solid var(--line);border-radius:8px;padding:8px'; n.value = bk.n || ''; n.oninput = () => { bk.n = n.value; renderCanvas(); }; box.appendChild(n); }
  else if (bk.t === 'espacio') { box.appendChild(lbl('Tamaño')); const sel = document.createElement('select'); sel.innerHTML = '<option value="sm">Chico</option><option value="md">Medio</option><option value="lg">Grande</option>'; sel.value = bk.alto || 'md'; sel.style.cssText = 'border:1px solid var(--line);border-radius:8px;padding:8px;width:100%'; sel.onchange = () => { bk.alto = sel.value; renderCanvas(); selectBlock(SEL); }; box.appendChild(sel); }
  else if (bk.t === 'separador') { box.appendChild(lbl('Grosor de la línea')); const sel = document.createElement('select'); sel.innerHTML = '<option value="2">Fina (2px)</option><option value="4">Media (4px)</option><option value="6">Gruesa (6px)</option><option value="8">Extra gruesa (8px)</option>'; sel.value = String(bk.grosor || 4); sel.style.cssText = 'border:1px solid var(--line);border-radius:8px;padding:8px;width:100%'; sel.onchange = () => { bk.grosor = parseInt(sel.value, 10); renderCanvas(); selectBlock(SEL); }; box.appendChild(sel); }
    ...
    const p = document.createElement('p'); p.className = 'fld-note'; p.style.margin = '6px 0 0'; p.textContent = 'El texto del botón se edita en el documento.'; box.appendChild(p);
  }
  else if (bk.t === 'html') { const p = document.createElement('p'); p.className = 'fld-note'; p.style.margin = '0'; p.textContent = 'Bloque de diseño del sistema. Podés moverlo o borrarlo, pero su contenido no se edita por bloques.'; box.appendChild(p); }
  else {
    const p = document.createElement('p'); p.className = 'fld-note'; p.style.margin = '0';
  ...
  const acts = document.createElement('div'); acts.className = 'gb-insp-acts';
  acts.innerHTML = '<button type="button" class="gb-i-up">↑ Subir</button><button type="button" class="gb-i-down">↓ Bajar</button><button type="button" class="gb-i-del">✕ Borrar</button>';
  acts.querySelector('.gb-i-up').onclick = () => moverBloque(SEL, -1);
  acts.querySelector('.gb-i-down').onclick = () => moverBloque(SEL, 1);
  acts.querySelector('.gb-i-del').onclick = () => borrarBloque(SEL);
  box.appendChild(acts);
}
function pasosInspector(bk) {
  const cont = document.createElement('div'); cont.style.cssText = 'display:flex;flex-direction:column;gap:8px';
  bk.steps.forEach((s, i) => {
    const row = document.createElement('div'); row.style.cssText = 'border:1px solid var(--line);border-radius:8px;padding:8px;display:flex;flex-direction:column;gap:6px';
    ...
    const del = document.createElement('button'); del.type = 'button'; del.textContent = 'Quitar paso'; del.style.cssText = 'border:none;background:none;color:var(--danger);cursor:pointer;font-size:12px;align-self:flex-start';
  ...
  const add = document.createElement('button'); add.type = 'button'; add.textContent = '+ Agregar paso'; add.style.cssText = 'border:1px dashed var(--line);background:none;border-radius:8px;padding:8px;cursor:pointer;color:var(--ink2);font-size:12.5px';
...
function imagenInspector(bk) {
  ...
  wrap.append(btn, inp);
  if (bk.src) { const prev = document.createElement('img'); prev.src = '/intranet/' + bk.src + '?t=' + Date.now(); prev.style.cssText = 'max-width:100%;border-radius:8px;border:1px solid var(--line)'; wrap.appendChild(prev); }
  ...
  const notaTam = document.createElement('p'); notaTam.className = 'fld-note'; notaTam.style.margin = '2px 0 0';
...
function galeriaInspector(bk) {
  ...
  const lista = document.createElement('div'); lista.style.cssText = 'display:flex;flex-direction:column;gap:5px';
  ...
  (bk.items || []).forEach((it, i) => {
    ...
    const nm = document.createElement('input'); nm.className = 'insp-input'; nm.style.flex = '1'; nm.placeholder = 'Nombre'; nm.value = it.nombre || '';
    ...
    const del = document.createElement('button'); del.type = 'button'; del.textContent = '✕'; del.title = 'Quitar'; del.className = 'gal-del';
  ...
  addMulti.style.cssText = 'border:1px dashed var(--line);background:none;border-radius:8px;padding:8px;cursor:pointer;color:var(--ink2);font-size:12.5px';
...
function pdfInspector(bk) {
  ...
  wrap.append(btn, inp);
  if (bk.src) { const p = document.createElement('div'); p.className = 'fld-note'; p.style.cssText = 'word-break:break-all;color:var(--ok)'; p.textContent = '✓ ' + bk.src.split('/').pop(); wrap.appendChild(p); }
...
function videoInspector(bk) {
  ...
  btn.onclick = () => inp.click();
  const estado = document.createElement('div'); estado.className = 'fld-note';
  estado.style.cssText = 'word-break:break-all';
  const pintarEstado = () => {
  ...
  const link = document.createElement('input'); link.type = 'text'; link.className = 'insp-inp';
  link.placeholder = 'https://youtube.com/watch?v=…';
  link.value = bk.url || '';
  link.style.cssText = 'width:100%;border:1px solid var(--line);border-radius:8px;padding:8px';
  link.oninput = () => {
...
function listaInspector(bk) {
  ...
  (bk.items || []).forEach((it, i) => {
    ...
    const card = document.createElement('div'); card.style.cssText = 'border:1px solid var(--line);border-radius:8px;padding:8px;display:flex;flex-direction:column;gap:6px';
    const head = document.createElement('div'); head.className = 'bk-row';
    const t = document.createElement('span'); t.style.cssText = 'flex:1;font-size:12px;color:var(--ink2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis'; t.textContent = txtOf(o.html) || 'Tarjeta ' + (i + 1);
    const del = document.createElement('button'); del.type = 'button'; del.textContent = '✕'; del.title = 'Quitar'; del.style.cssText = 'border:none;background:none;color:var(--danger);cursor:pointer;font-size:13px';
    del.onclick = () => { bk.items.splice(i, 1); if (!bk.items.length) bk.items.push({ icono: 'check', html: '' }); renderCanvas(); renderInspector(); };
    head.append(t, del);
    card.append(head, lbl('Ícono'), iconMini(o, 'icono'), lbl('Color del ícono'), colorPicker(o, 'color'));
    cont.appendChild(card);
  });
  const add = document.createElement('button'); add.type = 'button'; add.textContent = '+ Agregar tarjeta'; add.style.cssText = 'border:1px dashed var(--line);background:none;border-radius:8px;padding:8px;cursor:pointer;color:var(--ink2);font-size:12.5px';
...
/* ===================================================================
   ...
   =================================================================== */
function insCol() { const d = document.createElement('div'); d.style.cssText = 'display:flex;flex-direction:column;gap:8px'; return d; }
...
function insDel(fn) {
  const b = document.createElement('button'); b.type = 'button';
  b.className = 'insp-del'; b.textContent = '✕'; b.title = 'Quitar'; b.onclick = fn; return b;
}
function insHead(titulo, onDel) {
  const h = document.createElement('div'); h.className = 'bk-row';
  const t = document.createElement('span');
  t.style.cssText = 'flex:1;font-size:12px;font-weight:700;color:var(--ink2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis';
...
function insNota(html, cls) {
  ...
  p.className = 'fld-note' + (cls ? ' ' + cls : '');
  p.style.margin = '4px 0 0'; p.innerHTML = html; return p;
}
...
function chatInspector(bk, prop) {
  ...
  (bk[prop] || []).forEach((m, i) => {
    ...
    const row = document.createElement('div'); row.className = 'bk-row';
    const tog = document.createElement('button'); tog.type = 'button'; tog.className = 'btn btn-ghost'; tog.style.cssText = 'flex:1;font-size:12px;padding:6px 8px';
    ...
    const del = document.createElement('button'); del.type = 'button'; del.textContent = '✕'; del.title = 'Quitar'; del.style.cssText = 'border:none;background:none;color:var(--danger);cursor:pointer;font-size:13px';
    del.onclick = () => { bk[prop].splice(i, 1); if (!bk[prop].length) bk[prop].push({ lado: 'out', html: '' }); renderCanvas(); renderInspector(); };
    row.append(tog, del); cont.appendChild(row);
  });
  const addRow = document.createElement('div'); addRow.className = 'bk-row';
  const mk = (txt, lado) => { const b = document.createElement('button'); b.type = 'button'; b.textContent = txt; b.style.cssText = 'flex:1;border:1px dashed var(--line);background:none;border-radius:8px;padding:7px;cursor:pointer;font-size:12px;color:var(--ink2)'; b.onclick = () => { bk[prop].push({ lado, html: '' }); renderCanvas(); selectBlock(SEL); const el = $('#gbDoc').querySelector(`.gb-block[data-i="${SEL}"] [data-prop="${prop}"][data-idx="${bk[prop].length - 1}"]`); if (el) el.focus(); }; return b; };
...
function plantillaInspector(bk) {
  ...
  if (avisos.length) {
    const n = insNota('⚠️ ' + avisos.join('<br>'), 'insp-ayuda');
  ...
  switch (bk.t) {
    ...
    case 'video': {
      ...
      if (bk.src) {
        ...
        // preload=metadata: no baja el video entero hasta que lo tocan (dato del celular)
        return `<figure class="${clase}" style="${estilo}"><div class="v-box"><video src="${esc(bk.src)}" controls preload="metadata" playsinline${bk.poster ? ` poster="${esc(bk.poster)}"` : ''}></video></div>${pie}${dl}</figure>`;
      }
      if (bk.url) {
        const e = embedDeVideo(bk.url);
        if (!e.ok) return '';
...
function confirmar(msg, yesLabel, titulo, tono) {
  return new Promise(res => {
    ...
    m.hidden = false;
    const cerrar = v => { m.hidden = true; $('#confirmYes').onclick = $('#confirmNo').onclick = ov.onclick = null; res(v); };
...
function confirmarSalida() {
  return new Promise(res => {
    ...
    m.hidden = false;
    const cerrar = v => { m.hidden = true; $('#confirmAlt').hidden = true; $('#confirmNo').textContent = 'Cancelar'; $('#confirmYes').onclick = $('#confirmNo').onclick = $('#confirmAlt').onclick = ov.onclick = null; res(v); };
...
function estadoEditor() {
  ...
  return JSON.stringify({
    ...
    col: COLECCION, di: DOC_IDX, pal: COL_PALABRA
  });
}
...
function arrancarEdicion() {
  ultimoEstado = estadoEditor(); undoStack = []; actualizarUndoBtn(); actualizarBotones();
...
$('#imgLbClose').onclick = () => { $('#imgLightbox').hidden = true; };
$('#imgLightbox').onclick = e => { if (e.target.id === 'imgLightbox') $('#imgLightbox').hidden = true; };
document.addEventListener('keydown', e => { if (e.key === 'Escape' && !$('#imgLightbox').hidden) $('#imgLightbox').hidden = true; });
...
function galCard(img, sec) {
  ...
  c.innerHTML = `<button class="gx" type="button" title="Eliminar">✕</button>
...
async function guardarModulo({ salir = true, auto = false } = {}) {
  ...
  if (COLECCION) {
    ...
    if (sinNombre) { if (!auto) toast('Poné un nombre a cada ' + palabra() + ' (ej: Julio 2026)', 'err'); return false; }
    ...
    det.content = {
      ...
      docs: COLECCION.map(d => ({
        ...
        presentacion: !!d.presentacion,
        bloques: d.bloques || [],
        html: bloquesHTML(d.bloques || [], d.presentacion),
      })),
    };
  }
  else if (!galeriaMode) {
...
$('#detSave').onclick = () => guardarModulo({ salir: false });   // guarda y se queda en el editor
...
function cerrarBandeja() { $('#bandeja').hidden = true; }
...
async function chequearActualizacion() {
  if (ES_CENTRAL) return;                       // la central la actualiza el desarrollador
  let st;
...
function esperarReinicio() {
  ...
  const t = setInterval(async () => {
    ...
    try {
      ...
      if (cayo) { clearInterval(t); location.reload(); }   // volvio -> cargar la version nueva
    } catch (e) { cayo = true; }                            // se esta reiniciando
