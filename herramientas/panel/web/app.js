...
function buscarEnTodo(q) {
  ...
  MODULOS.forEach((m, idx) => {
    ...
    if (c.tipo === 'coleccion' || c.tipo === 'muro') (c.docs || []).forEach((d, di) => mirar(m, idx, d.bloques, di, d.titulo));
...
async function renderContenidoEditor() {
  ...
  if (det.content && det.content.tipo === 'muro') {
...
function pintarModo() {
  ...
  const am = $('#apMeta');
  if (am) {
    const nm = { pagina: 'Página', presentacion: 'Presentación', biblioteca: 'Biblioteca', muro: 'Muro' }[modo] || '';
    ...
    card.innerHTML = `
      ...
      <div class="col-info">
        <div class="col-t">${esc(d.titulo || 'Sin nombre')}
          ${(ES_MURO && d.fijado && !d.archivado) ? '<span class="col-badge fij">📌 Fijada</span>' : ''}
...
function kitVisorHTML(paquete, fecha) {
  ...
  return [
    ...
    '  c.appendChild(h2("Para el desarrollador: trabajar con IA"));',
...
function publicacionesDelMuro() {
  const mods = MODULOS || [];
  const muro = mods.find(m => m.content && m.content.tipo === 'muro' && !m.hidden);
  if (!muro) return [];
  return (muro.content.docs || []).filter(d => !d.archivado);
}

function abrirMetricas() {
  $('#viewModulos').hidden = true;
