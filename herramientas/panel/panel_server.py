...
VERSION_PUBLICA = "1.29.0"
VERSION_LABEL = "1.29.0 - tutoriales con buscador y temas al costado"
VERSION_NOTES = (
                 "TUTORIALES CON BUSCADOR Y LOS TEMAS AL COSTADO: la pantalla de "
                 "tutoriales se rehizo con la forma de las plataformas de cursos. El "
                 "video queda grande a la izquierda y al lado, siempre a la vista, "
                 "la lista de temas del video, con el que esta sonando resaltado; se "
                 "toca un tema y el video salta ahi, sin perder el video de vista "
                 "(antes los temas estaban debajo del video y habia que bajar la "
                 "pantalla). Debajo de los temas aparecen los otros tutoriales, para "
                 "pasar de uno a otro sin volver atras. Y arriba hay una lupa: se "
                 "escribe por ejemplo modulo y aparecen los tutoriales que hablan de "
                 "eso, incluso mostrando el minuto exacto donde se habla del tema, "
                 "con un toque para entrar justo ahi. Busca en el titulo, en la "
                 "descripcion y en el texto de cada tema. En pantallas angostas la "
                 "lista de temas se acomoda abajo del video.")
...
def kit_recuperacion():
    ...
    return {
        ...
        # La clave de publicacion viaja ESCRITA en el archivo, por decision del
        # dueno del sistema: el archivo es el traspaso, y prefirio que quien lo
        # reciba tenga todo junto sin depender de buscar en otra computadora.
        # El archivo ya no va cifrado (una contrasena perdida lo volvia inutil),
...
# Quien firma una publicacion. Vive en los ajustes GLOBALES y no en esta
# computadora: guardado local, cada sucursal tendria su propia lista y la misma
# publicacion saldria firmada distinto segun de que maquina se cargo.
SECTOR_PRINCIPAL = "Marketing"
SECTORES_POR_DEFECTO = [SECTOR_PRINCIPAL, "Administración", "Dirección",
                        "Tapicería", "Depósito y Entregas"]
AJUSTES_POR_DEFECTO = {"novedad_horas": 24,
                       "sectores": list(SECTORES_POR_DEFECTO)}
...
def validar_ajustes(d):
    ...
    # El principal va siempre y va primero. Una lista vacia SI se respeta
    # (alguien dejo solo Marketing a proposito); lo que se repone por defecto
    # es cuando el campo no vino, que es la primera vez que se guarda.
    sec = d.get("sectores")
    if isinstance(sec, list):
        limpio = [SECTOR_PRINCIPAL]
        for x in sec:
            x = str(x if x is not None else "").strip()
            if x and len(x) <= 40 and x not in limpio:
                limpio.append(x)
    else:
        limpio = list(SECTORES_POR_DEFECTO)
    return {"novedad_horas": h, "sectores": limpio}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _datos_post(self, path):
        ...
        if path == "/api/datos/informe-editar":
            rep = datos_api.buscar(cfg, str(cuerpo.get("id") or ""))
            if not rep:
                return self._json({"error": "no encuentro ese reporte"}, 404)
            ops = cuerpo.get("opciones")
