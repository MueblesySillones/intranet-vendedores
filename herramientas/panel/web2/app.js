...
function abrirModal(m) {
  ...
  m.classList.remove('yendose');
  m.hidden = false;
}
function esconderModal(m) {
  if (!m || m.hidden) return;
  if (sinMovimiento()) { m.hidden = true; return; }
  const mio = m._cierre = (m._cierre || 0) + 1;
  m.classList.add('yendose');
  setTimeout(() => {
    if (m._cierre !== mio) return;         // lo reabrieron en el medio
    m.hidden = true; m.classList.remove('yendose');
  }, 150);
}
...
async function conBoton(sel, trabajando, accion, { quieto = false } = {}) {
  ...
  const antes = bs.map(b => b.textContent);
  bs.forEach(b => {
    b.dataset.ocupado = '1';
    b.disabled = true;
    if (trabajando) b.textContent = trabajando;
  });
  ...
  try {
    ...
    bs.forEach((b, i) => {
      ...
      b.disabled = false;
      if (trabajando) b.textContent = antes[i];
    });
  }
}
...
function pintarModulos() {
  ...
  const h = document.querySelector('#viewModulos .lz-head h2');
  if (h) h.textContent = 'Módulos';
...
function modCard(m, idx, pos) {
  ...
  c.innerHTML = `
    <span class="mod-grip" title="Arrastrá para ordenar"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true" focusable="false"><circle cx="9" cy="6" r="1.5"/><circle cx="15" cy="6" r="1.5"/><circle cx="9" cy="12" r="1.5"/><circle cx="15" cy="12" r="1.5"/><circle cx="9" cy="18" r="1.5"/><circle cx="15" cy="18" r="1.5"/></svg></span>
    <span class="icon-pos">${pos == null ? idx + 1 : pos}</span>
...
function mostrarDetalle(on) {
  ...
  if (on) document.querySelectorAll('.lienzo > .main').forEach(v => {
...
function actualizarBotones() {
  ...
  if (sb) {
    ...
    sb.classList.remove('btn-done');
    sb.classList.toggle('active', dirty);
  }
  const pb = $('#detPublicar');
  if (pb) {
    pb.textContent = 'Publicar';
    pb.classList.add('btn-pub'); pb.classList.remove('btn-done');
  }
  const ch = $('#detEstado');
  if (ch) {
    // con cambios sin guardar no hay nada que declarar: el chip se esconde
    if (dirty) ch.hidden = true;
    else { ch.hidden = false; ch.textContent = pubPend ? 'Guardado ✓' : 'Publicado ✓'; }
  }
...
document.addEventListener('keydown', e => {
  ...
  for (const id of ['histModal', 'avisarModal']) {
    const m = document.getElementById(id);
    if (m && !m.hidden) { const x = m.querySelector('[data-cerrar-hist], [data-cerrar-avisar]'); if (x) x.click(); return; }
  }
});
