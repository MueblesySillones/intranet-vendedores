...
(function () {
  ...
  const SECS = ['muro', 'modulos', 'datos', 'metricas'];
  let SEC = 'muro';
  let FILTRO = 'todas';
  ...
  document.getElementById('navSecs').addEventListener('click', e => {
    const b = e.target.closest('.nav-s'); if (b) irASeccion(b.dataset.sec);
  });
  ...
  async function restaurar(k) {
    ...
    renderMuro();
    pintarContadores();
    toast('Volvió al muro. Fijate la fecha, porque entró arriba de todo.', 'ok');
  }
  ...
  function tarjetaPost(d, i) {
    ...
    const cuerpo = txtOf(d.html || '').slice(0, 220);
    const chips = [];
    if (d.fijado && est !== 'oculta') chips.push('<span class="mp-chip fij"><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M12 17v4M8 4h8l-1 6 3 3H6l3-3z"/></svg> Fijada</span>');
    if (est === 'programada') chips.push('<span class="mp-chip prog">Sale ' + esc(cuando(d)) + '</span>');
    if (est === 'vencida') chips.push('<span class="mp-chip venc">Ya no se ve</span>');
    if (est === 'oculta') chips.push('<span class="mp-chip ocul">Oculta</span>');
    if (d.confirmar) chips.push('<span class="mp-chip conf">Pide confirmación</span>');
    if (d.archivar) {
      var mArch = (MODULOS || []).filter(function (m) { return m.key === d.archivar; })[0];
      chips.push('<span class="mp-chip arch">También en ' +
        esc(mArch ? (mArch.title || d.archivar) : d.archivar) + '</span>');
    }
    if (d.vence && est === 'publicada') {
      const q = diasHasta(d.vence);
      if (q !== null && q >= 0) chips.push('<span class="mp-chip venc">' +
        (q === 0 ? 'último día' : q <= 7 ? 'vence en ' + q + ' días' : 'hasta ' + esc(fechaCorta(d.vence))) + '</span>');
    }
    const el = document.createElement('article');
    el.className = 'mp' + (d.fijado ? ' fijada' : '') + (est === 'vencida' ? ' vencida' : '') +
      (est === 'oculta' ? ' oculta' : '');
    el.dataset.i = i;
    el.innerHTML = `
      <div class="mp-acts">
        <button type="button" class="mp-mas" data-a="menu" aria-haspopup="true"
          aria-expanded="false" title="Más opciones" aria-label="Más opciones">
          <svg viewBox="0 0 24 24" fill="currentColor"><circle cx="5" cy="12" r="1.9"/><circle cx="12" cy="12" r="1.9"/><circle cx="19" cy="12" r="1.9"/></svg>
        </button>
      </div>
      <!-- el encabezado (autor, fecha, tipo) lo trae la vista real de abajo -->
      ${chips.length ? `<div class="mp-pie">${chips.join('')}</div>` : ''}`;
    /* La publicación se ve TAL CUAL la ve el vendedor: así, al cargarla, se
       corrige de una en vez de descubrir el problema una vez publicada. */
    el.insertBefore(montarComoSea(d), el.querySelector('.mp-pie') || null);
    el.addEventListener('click', ev => {
      const mas = ev.target.closest('.mp-mas');
      if (mas) { ev.stopPropagation(); abrirMenuPost(mas, d, i, el); return; }
      const b = ev.target.closest('.mp-a');
      if (b) { ev.stopPropagation(); accionPost(b.dataset.a, i, el); return; }
      /* ⚠️ Tocar la publicación NO abre el editor. Antes sí, y con la vista
         real quedó tosco: darle play al video o tocar una foto te sacaba de
         la pantalla sin haberlo pedido. Para editar está el menú de los tres
         puntitos, que es donde uno va cuando quiere cambiar algo. */
    });
    return el;
  }
  ...
  function abrirMenuPost(boton, d, i, el) {
    ...
    var opciones = [
      ...
      { a: 'ocultar', t: d.archivado ? 'Volver a mostrarla' : 'Ocultar del muro',
  ...
  function renderMuro() {
    ...
    const filtros = document.getElementById('muroFiltros');
    const sub = document.getElementById('muroSub');
    document.getElementById('compAv').textContent = iniciales('Marketing');

    if (!mod) {
      filtros.innerHTML = '';
      cont.innerHTML = `<div class="vacio"><b>Todavía no hay un muro</b>
        <p>El muro es la portada de la intranet: lo primero que ven los vendedores al entrar.</p>
        <button type="button" class="btn active" id="crearMuro">Crear el muro</button></div>`;
      document.getElementById('crearMuro').onclick = crearMuro;
      sub.textContent = 'Todavía no está creado.';
      return;
    }
    ...
    const vivas = cuenta.todas - cuenta.ocultas - cuenta.vencidas;
    sub.textContent = vivas === 1 ? '1 publicación a la vista de los vendedores'
      : vivas + ' publicaciones a la vista de los vendedores';

    const defs = [['todas', 'Todas'], ['programadas', 'Programadas'], ['fijadas', 'Fijadas'],
                  ['vencidas', 'Vencidas'], ['ocultas', 'Ocultas']];
    const tacho = enPapelera();
    filtros.innerHTML = defs.filter(f => f[0] === 'todas' || cuenta[f[0]]).map(f =>
      `<button type="button" class="mf${FILTRO === f[0] ? ' on' : ''}" data-f="${f[0]}">${f[1]}<b>${cuenta[f[0]]}</b></button>`).join('') +
      (tacho.length
        ? `<button type="button" class="mf mf-tacho${FILTRO === 'papelera' ? ' on' : ''}" data-f="papelera">Papelera<b>${tacho.length}</b></button>`
        : '');

    /* la papelera es otra lista: se pinta aparte y se sale de acá */
    if (FILTRO === 'papelera') {
      sub.textContent = tacho.length === 1 ? '1 publicación eliminada, todavía recuperable'
                                           : tacho.length + ' publicaciones eliminadas, todavía recuperables';
      pintarPapelera(cont, tacho);
      return;
    }
    ...
    if (!listados.length) {
      cont.innerHTML = `<div class="vacio"><b>${q ? 'No encontré nada con eso' : 'Nada por acá'}</b>
  ...
  document.getElementById('muroFiltros').addEventListener('click', e => {
    const b = e.target.closest('.mf'); if (!b) return;
    FILTRO = b.dataset.f; renderMuro();
  });
  ...
  function marcarModo() {
    ...
    elCo('coPublicar').textContent = editando ? 'Guardar y publicar' : 'Publicar';
  ...
  async function editarPublicacion(i) {
    ...
    document.getElementById('composer').classList.add('abierto');
    elCo('compOpen').hidden = false;
    ...
    pintarTipos();
    pintarAdjuntos();
    elCo('compOpen').scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    setTimeout(function () { elCo('coTitulo').focus(); }, 60);
  }
  ...
  function cerrarComp() {
    ...
    COMP.editando = null;
    marcarModo();
    document.getElementById('composer').classList.remove('abierto');
    elCo('compOpen').hidden = true;
    cerrarPicker();
  }
  ...
  function pintarTipos() {
    elCo('coTipos').innerHTML = Object.keys(TIPOS).map(function (k, i) {
      var v = TIPOS[k];
      return '<button type="button" class="co-tipo' + (COMP.tipo === k ? ' on' : '') +
        '" data-t="' + k + '" style="--c:' + v.c + ';--rgb:' + rgbDe(v.c) +
        ';--i:' + i + '"><span class="ct-p"></span>' + v.t + '</button>';
    }).join('');
    elCo('coTipos').querySelectorAll('.co-tipo').forEach(function (b) {
      b.onclick = function () {
  ...
  function pintarAdjuntos() {
    ...
    cont.innerHTML = COMP.bloques.map(function (bk, i) {
      var detalle = '', miniat = '', its;
      if (bk.t === 'imagen') detalle = bk.src ? 'lista' : 'sin foto';
      if (bk.t === 'galeria') {
        its = (bk.items || []).filter(function (it) { return it.src; });
        detalle = its.length + (its.length === 1 ? ' foto' : ' fotos');
        /* el panel se sirve desde web2/: una ruta relativa "assets/..." da 404.
           Los archivos del proyecto se ven bajo /intranet/, igual que en el
           canvas del editor de modulos. */
        miniat = '<span class="co-adj-miniat">' + its.slice(0, 4).map(function (it) {
          return '<img src="/intranet/' + esc(it.src) + '" alt="" loading="lazy">';
        }).join('') + '</span>';
      }
      if (bk.t === 'video') detalle = bk.src ? 'listo' : 'sin video';
      if (bk.t === 'pdf') detalle = bk.nombre || 'sin archivo';
      if (bk.t === 'lista') detalle = (bk.items || []).length + ' ítems';
      if (bk.t === 'ref') detalle = [bk.mod, bk.sub].filter(Boolean).join(' › ') || 'sin elegir';
      ...
      return '<div class="co-adj' + (AD_NOMBRE[bk.t] ? '' : ' ajeno') + '"><b>' +
        (AD_NOMBRE[bk.t] || nombreBloque(bk.t)) + '</b>' +
        '<span>' + esc(detalle) + '</span>' + miniat +
        '<button type="button" class="co-adj-x" data-i="' + i + '" title="Sacar">&times;</button></div>';
    }).join('');
  ...
  function pintarContadores() {
    ...
    const n = (MODULOS || []).filter(m => !m.hidden).length;
    nd.hidden = !n; nd.textContent = n;
  }
  window.pintarContadores = pintarContadores;

  /* al arrancar: la sección Muro es la casa */
